...
public class LandFrame extends JFrame {
	...
	public LandFrame() {
		// 首先设置窗口的相关信息
		super();// 调用父类的构造方法
		setTitle(" 第七组");// 设置窗口的标题
		setResizable(true);// 设置窗口不可以改变大小
		...
		passwordField.setColumns(10);// 设置密码框可显示的字符数
	...
	class LandButtonActionListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			String username = usernameComboBox.getSelectedItem().toString();// 获得登录用户的名称
				try {
					System.out.print(MD5.EncoderByMd5("123"));
			...
			try{
			String inputPassword =MD5.EncoderByMd5(turnCharsToString(passwords));// 将密码从char型数组转换成字符串
			if (username.equals("TSoft")) {// 查看是否为默认用户登录
				...
				if (inputPassword.equals(password)) {// 查看登录密码是否正确
					...
					JOptionPane.showMessageDialog(null, "登录密码错误，请确认后重新登录！您还有"+i+"次机会",
